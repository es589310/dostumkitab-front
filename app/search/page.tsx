...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const searchBooks = async () => {
      try {
        // Önce arama teriminin bir kategori adı olup olmadığını kontrol et
        ...
        // Eğer kategori adı bulunduysa, sadece kategori ID'si ile arama yap
        if (matchingCategory) {
          searchParams.category = matchingCategory.id.toString()
        } else {
          // Kategori adı bulunmadıysa, genel arama yap
          searchParams.search = query
        }

        const data: BooksResponse = await api.getBooks(searchParams)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="text-2xl font-bold mb-6">Axtarış nəticələri: <span className="text-blue-600">{query}</span></h1>
      {loading && <div>Yüklənir...</div>}
      {error && <div className="text-red-500">{error}</div>}
      {!loading && !error && books.length === 0 && (
        <div className="text-gray-500 text-lg">Axtardığınız nəticə tapılmadı</div>
      )}
